fn main() {
    let input = "nkzjrdqrmpztpqninetwofour1znnkd
    ...
    let result1 = first_part(input);
    println!("result 1: {:?}", result1);

    let result2 = second_part(input);
    println!("result 2: {:?}", result2);
}

fn first_part(text: &str) -> i32 {
    let no_letters: Vec<char> = text
        .chars()
        .filter_map(|a| -> Option<char> {
            if a.is_numeric() || a == '\n' {
                Option::from(a)
            } else {
                None
    ...
    let joined: String = no_letters.iter().collect::<String>();

    let split: Vec<&str> = joined.split('\n').collect::<Vec<&str>>();

    let extract: Vec<String> = split
        .into_iter()
        .map(|a| {
            let first_char = a.to_string().chars().nth(0).unwrap().to_string();
            let last_char = a.to_string().chars().nth(a.len() - 1).unwrap().to_string();
            format!("{}{}", first_char, last_char)
        })
        .collect::<Vec<String>>();

    let to_int: Vec<i32> = extract
        .into_iter()
        .map(|a| a.parse::<i32>().unwrap())
        .collect::<Vec<i32>>();

    let sum: i32 = to_int.iter().sum();
    sum
}

fn second_part(text: &str) -> i32 {
    let convert_numeric_letters_to_digits = text
        .replace("one", "o1e")
        .replace("two", "t2o")
        .replace("three", "t3e")
        .replace("four", "f4r")
    ...
    first_part(convert_numeric_letters_to_digits.as_str())
}
